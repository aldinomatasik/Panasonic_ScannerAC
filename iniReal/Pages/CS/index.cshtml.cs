...
END;";
        ...
        // ============================================================
        // TAMBAHKAN 2 METHOD INI KE DALAM class IndexModel di CS/Index.cshtml.cs
        // (taruh di deket OnPostAsync, sebelum CountDataAddedToday juga boleh)
        // ============================================================

        // ─── GET MISSING SERIALS (dipanggil oleh showShiftEndModal -> fetchCsMissingSerials) ───
        public async Task<IActionResult> OnGetMissingSerialsAsync(string machineCode, string startTime, string endTime)
        {
            ...
            {
                Console.WriteLine("Exception OnGetMissingSerialsAsync (CS): " + ex.ToString());
                return new JsonResult(new { error = ex.Message });
            }
        }

        // ─── ADD MISSING SERIAL (dipanggil oleh addCsSerial di JS) ───
        public class AddCsSerialRequest
        {
        ...
        public async Task<IActionResult> OnPostAddSerialAsync([FromBody] AddCsSerialRequest req)
        {
            try
            {
            ...
            {
                Console.WriteLine("Exception OnPostAddSerialAsync (CS): " + ex.ToString());
